...
void drv_task(void *param) {
  while (1) {
    ...
    while (!(evt_bits & EVT_READ_REQUEST)) {
      evt_bits = xEventGroupWaitBits(events, EVT_READ_REQUEST, pdTRUE, pdTRUE,
                                     UINT32_MAX);
    }
    ...
    balance =
        ((temp & 0x00FF) + (temp >> 8) + (humid & 0x00FF) + (humid >> 8)) &
        0x00FF;
 ...
 */
void timer_cb_cache(void *param) { flags |= FLAG_REFRESH; }
...
void dht_init(uint32_t data_pin) {
  ...
  xTaskCreatePinnedToCore(drv_task, "drv_sensor", 2048, NULL, PRIO_DRIVER,
                          &drv_sensor, 0);
...
int dht_read(dht_data_t *out) {
  ...
  bits = xEventGroupWaitBits(events, EVT_READ_FAILURE | EVT_READ_COMPLETE,
                             pdTRUE, pdFALSE, UINT32_MAX);
