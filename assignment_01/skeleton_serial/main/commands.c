...
int process_cmd_status(const Command *in_cmd, char *out_msg) {
  ...
  asprintf(
    &time_string, 
    "SYSTEM UPTIME: %d",
    (int)(esp_timer_get_time() * 0.000001)
  );
...
int valid_num(char* in, char top) {
...
int process_cmd_dec(const Command *in_cmd, char *out_msg) {
  char f = in_cmd->argument[0]; // First 
  char s = in_cmd->argument[1]; // Second
  long dec;

  if(f == '0' && s == 'x') {
    if (valid_num(&in_cmd->argument[2], 'f') == 0) {
      strcpy(out_msg, "\nARGUMENT ERROR");
      return 0;
    }

    dec = strtol(in_cmd->argument, NULL, 16);
  } else if (f == '0' && s == 'b') {
    if (valid_num(&in_cmd->argument[2], '1') == 0) {
      strcpy(out_msg, "\nARGUMENT ERROR");
      return 0;
    }

    char* sub = &in_cmd->argument[2];
    dec = strtol(sub, NULL, 2);
  } else if (f == '0') {
    if (valid_num(&in_cmd->argument[1], '7') == 0) {
      strcpy(out_msg, "\nARGUMENT ERROR");
      return 0;
    }

    dec = strtol(in_cmd->argument, NULL, 8);
  } else {
    if(valid_num(in_cmd->argument, '9') == 0) {
      strcpy(out_msg, "\nARGUMENT ERROR");
      return 0;
    }
    dec = strtol(in_cmd->argument, NULL, 10);
  }
