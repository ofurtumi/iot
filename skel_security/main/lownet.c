...
void lownet_init(lownet_recv_fn receive_cb, lownet_cipher_fn encrypt_fn,
  ...
  xTaskCreatePinnedToCore(lownet_service_main, "lownet_service", 2048,
                          NULL, // Pass no params into task method.
                          LOWNET_SERVICE_PRIO, &net_system.service,
                          LOWNET_SERVICE_CORE);
...
void lownet_service_main(void *pvTaskParam) {
  ...
  while (1) {
    ...
    if (xQueueReceive(net_system.inbound, &frame, UINT32_MAX) == pdTRUE) {
      ...
      // source
      //	address, the broadcast address, discard it -- something has gone
      //wrong.
